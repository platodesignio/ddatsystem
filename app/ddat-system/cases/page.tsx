...
import { DDATSignal } from '@/lib/ddat/schema'
        ...
        <div className="flex items-center justify-between mb-6 gap-4">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="px-3 py-2 text-xs border border-white/20 text-gray-400 rounded hover:border-white/40 hover:text-white transition-colors"
            >
              Reset Demo Cases
            </button>
            <Link
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/20">
                    {['Case ID', 'Title', 'Domain', 'DDAT Signal', 'Red', 'Unknown', 'Evidence', 'Status', 'Updated', ''].map((h) => (
                  ...
                  {filtered.length === 0 ? (
                    <tr>
                      <td colSpan={10} className="px-4 py-8 text-center text-gray-500">
                        {cases.length === 0
                          ? 'No cases found. Demo cases failed to load — click Reset Demo Cases.'
                    ...
                    filtered.map((c, i) => (
                      <tr
                        key={c.id}
                        className={`border-b border-white/5 ${i % 2 === 0 ? 'bg-white/5' : ''} hover:bg-white/10 transition-colors`}
                      >
